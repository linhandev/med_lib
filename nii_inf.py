...
class Predictor:
    ...
    def output_result(self, imgs_data, infer_out, use_pr=False):
        for idx in range(len(imgs_data)):
            ...
            #     result_png, ori_shape, fx=0, fy=0, interpolation=cv2.INTER_CUBIC
            # )
    ...
    def predict(self, images):
        ...
        for i in range(0, len(images), batch_size):
            ...
            img_datas = self.image_reader.process(
                images[i : i + real_batch_size], use_pr
            )
...
def run(deploy_conf, imgs_dir, support_extensions=".jpg|.jpeg"):
    ...
    if len(imgs) == 0:
        print(
            "No Image (with extensions : %s) found in [%s]"
