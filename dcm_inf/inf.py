...
class DeployConfig:
    def __init__(self, conf_file):
        ...
        with open(conf_file) as fp:
            ...
            self.model_file = os.path.join(deploy_conf["MODEL_PATH"], deploy_conf["MODEL_FILENAME"])
            self.param_file = os.path.join(deploy_conf["MODEL_PATH"], deploy_conf["PARAMS_FILENAME"])
            # 6. use_gpu
...
class ImageReader:
    ...
    def process_worker(self, imgs, idx, use_pr=False):
        ...
        if ori_h != eval_crop_size[1] or ori_w != eval_crop_size[0]:
            im = cv2.resize(im, eval_crop_size, fx=0, fy=0, interpolation=cv2.INTER_LINEAR)

        # if use models with no pre-processing/post-processing op optimizations
        if not use_pr:
            im_mean = np.array(self.config.mean).reshape((self.config.channels, 1, 1))
            im_std = np.array(self.config.std).reshape((self.config.channels, 1, 1))
    ...
    def process(self, imgs, use_pr=False):
        ...
        with ThreadPoolExecutor(max_workers=self.config.batch_size) as exe_pool:
            tasks = [exe_pool.submit(self.process_worker, imgs, idx, use_pr) for idx in range(len(imgs))]
...
class Predictor:
    def __init__(self, conf_file):
        ...
        if self.config.predictor_mode == "NATIVE":
            ...
            predictor_config = fluid.core.AnalysisConfig(self.config.model_file, self.config.param_file)
    ...
    def output_result(self, imgs_data, infer_out, use_pr=False):
        for idx in range(len(imgs_data)):
            ...
            # if not use_pr:
            result_png = cv2.resize(result_png, ori_shape, fx=0, fy=0, interpolation=cv2.INTER_CUBIC)
    ...
    def predict(self, images):
        ...
        for i in range(0, len(images), batch_size):
            ...
            img_datas = self.image_reader.process(images[i : i + real_batch_size], use_pr)
            input_data = np.concatenate([item[1] for item in img_datas])
            input_data = self.create_tensor(input_data, real_batch_size, use_pr=use_pr)
...
def run(deploy_conf, imgs_dir, support_extensions=".jpg|.jpeg"):
    ...
    if len(imgs) == 0:
        print("No Image (with extensions : %s) found in [%s]" % (support_extensions, imgs_dir))
        return -1
